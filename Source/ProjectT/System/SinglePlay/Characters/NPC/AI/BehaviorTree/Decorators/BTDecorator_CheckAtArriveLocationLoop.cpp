...
#include "BTDecorator_CheckAtArriveLocationLoop.h"
...
UBTDecorator_CheckAtArriveLocationLoop::UBTDecorator_CheckAtArriveLocationLoop() :
	AcceptRadius(1.f),
	bIncludeCapsuleRadius(false)
{
	NodeName = "Check At ArriveLocation Loop";
}

bool UBTDecorator_CheckAtArriveLocationLoop::CalculateRawConditionValue(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) const
{
	...
	if (BlackboardKey.SelectedKeyType == UBlackboardKeyType_Vector::StaticClass())
	{
		...
		NMT_LOGF("Check At ArriveLocation Loop. Dist %f", Distance2D)
